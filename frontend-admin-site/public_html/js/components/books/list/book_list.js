        ...
        .controller('BooksListCtrl', [
            ...
            function ($scope, Util, API, $state, AppConfig) {
                ...
                $scope.loadListProduct = function (sortCase) {
                    ...
                    Util.createRequest(API.GET_LIST_BOOK, param, function (response) {
                            ...
                            if ($scope.selected.length > 0) {
                                ...
                                $scope.listProduct.forEach(function (result) {
                                    $scope.selected.forEach(function (selected) {
                                        if (result.bookId === selected.bookId) {
                ...
                $scope.confirmDeleteProduct = function (bookId) {
                    var listId = [];
                    ...
                    Util.showConfirmModal({
                        ...
                        var bookIds = _.map($scope.selected, 'bookId');
                        if (bookId) {
                            listId.push(bookId);
                        } else {
                            listId = bookIds;
                        ...
                        Util.createRequest(API.DELETE_BOOK, listId, function (response) {
                ...
                $scope.toggleSelectAll = function (isSelectAll) {
                    if (isSelectAll) {
                        if ($scope.selected.length === 0) {
                            ...
                            for (var i = 0; i < $scope.selected.length; ++i) {
                                for (var j = i + 1; j < $scope.selected.length; ++j) {
                                    if ($scope.selected[i].bookId === $scope.selected[j].bookId)
                        ...
                        $scope.listProduct.forEach(function (item) {
                            $scope.selected.forEach(function (selected) {
                                if (item.bookId === selected.bookId)
                ...
                $scope.toggleSelected = function (obj) {
                    ...
                    if (angular.isObject(obj) && obj.bookId) {
                        console.log("$scope.selected", $scope.selected);
                        var index = _.findIndex($scope.selected, {bookId: obj.bookId});
                        if (index > -1) {
                ...
                $scope.loadProductDetail = function (bookId) {
                    $state.go('books.detail', {id: bookId});
                };
