...
//                                                console.log(value.url);
    ...
    .directive('ngImgUpload', ['$parse', '$http', 'AppConfig', '$rootScope', 'Util', '$base64', function ($parse, $http, AppConfig, $rootScope, Util, $base64) {
        return {
            restrict: 'A',
            link: function (scope, element, attrs) {
                var model = $parse(attrs.ngImgUpload);
                var modelSetter = model.assign;
                element.bind('change', function () {
//                            console.log("dcm");
                    var value = {};
                    setTimeout(function () {
                        angular.forEach(element[0].files, function (item) {
                            if (item.size < (1024 * 1024 * 10)) {
                                ...
                                setTimeout(function () {
                                    //console.log('modal',element[0].placeholder);
                                    value = {
                                        type: element[0].placeholder,
    ...
    .directive('imgUpload', ['$parse', '$rootScope', '$base64',function ($parse, $rootScope, $base64) {
        return {
            restrict: 'A',
            require: '?ngModel',
            link: function (scope, elem, attrs) {
                ...
                elem.on('change', function () {
                    ...
                    if(extensions.indexOf(extension) == -1){
                        ...
                        scope.file = {};
                        scope.file._file = elem[0].files[0];
                        scope.file.imageName = filename;
                        scope.file.base64 = $base64.encode(image);;
